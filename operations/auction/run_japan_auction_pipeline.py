#!/usr/bin/env python3
"""
Japan AUCTION LISTINGS pipeline orchestrator (inventory / live listings, not sold-results).

1. Clean: Remove expired lots (pipeline/1_remove_lots.py)
2. Listings: Extract auction listings (pipeline/extract_auction_listings.py)
...
def run_listings(site: str | None = None, maker: str | None = None, limit: int = 0) -> int:
    """Run listing extraction (extract_auction_listings.py). UID-based skip built in."""
    cmd = [sys.executable, "-u", str(_script_dir / "pipeline" / "extract_auction_listings.py")]
    if site:
